...
export const PianoKeyboard = ({
  ...
  // Compute active visible keys (wide keys by default for realistic width, or full 88)
  const startMidi = (startOctave + 1) * 12; // e.g. C3 is MIDI 48
  const endMidi = startMidi + octaveCount * 12 - 1;

  const visibleKeys = full88Mode
    ? ALL_PIANO_KEYS
    : ALL_PIANO_KEYS.filter((k) => k.midi >= startMidi && k.midi <= endMidi);
  ...
  return (
    ...
    >
      ...
      {/* Octave Shift Strip (Hidden in pure Normal Piano mode for zero text) */}
      {!isNormalMode && (
      ...
      <div className="piano-keys-bed">
        {/* Authentic Red Felt Damper Cloth running along keybed */}
        <div className="piano-red-felt-cloth"></div>

        {/* Left Wooden Key Cheek Block */}
        {isNormalMode && (
          <div className="piano-cheekblock cheekblock-left">
            {!full88Mode && (
              <button
                className="cheekblock-oct-btn"
                disabled={startOctave <= 1}
                onClick={() => onShiftOctave(Math.max(1, startOctave - 1))}
                title="Shift Octave Down (Bass)"
              >
                ◀
              </button>
            )}
        ...
        {/* Key Surface with absolute black keys positioned over white keys */}
        <div className="piano-keys-surface">
          {/* White Keys Layer */}
        ...
        {/* Right Wooden Key Cheek Block */}
        {isNormalMode && (
          <div className="piano-cheekblock cheekblock-right">
            {!full88Mode && (
              <button
                className="cheekblock-oct-btn"
                disabled={startOctave >= 5}
                onClick={() => onShiftOctave(Math.min(5, startOctave + 1))}
                title="Shift Octave Up (Treble)"
              >
                ▶
              </button>
            )}
